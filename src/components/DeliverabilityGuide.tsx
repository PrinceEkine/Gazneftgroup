      ...
      >
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Deliverability Guide</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Ensure your emails land in the inbox, not spam.</p>
            </div>
          </div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-4 rounded-xl">
                <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">SPF (Sender Policy Framework)</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Specifies which IP addresses and domains are allowed to send emails on your behalf.</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-4 rounded-xl">
                <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">DKIM (DomainKeys Identified Mail)</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Adds a digital signature to your emails, proving they weren't tampered with in transit.</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-4 rounded-xl">
                <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">DMARC (Domain-based Message Authentication)</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Tells receiving servers what to do if SPF or DKIM fails (e.g., "quarantine" or "reject").</p>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            <ul className="space-y-3">
              <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                <div className="mt-1"><CheckCircle2 size={14} className="text-green-600 dark:text-green-500" /></div>
          ...
          <section className="space-y-4">
            ...
            <div className="bg-blue-600/5 border border-blue-500/20 p-4 rounded-xl space-y-3">
              <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                ...
                You can check your IP reputation on sites like <a href="https://mxtoolbox.com/blacklists.aspx" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1">MXToolbox <ExternalLink size={12} /></a>.
              </p>
              <div className="pt-3 border-t border-blue-500/10">
                <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase mb-1">IP Warm-up</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">If you have a new IP, start by sending small volumes of email and gradually increase it over several weeks to build trust with ISPs.</p>
              </div>
            </div>
          </section>
